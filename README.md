...
//аргумент под названием state - объект, который описывает состояние всего нашего приложения
...
//Actions - для изменения состояния стора. Описывают, что произошло, но не описывают, как изменяется состояние приложения. объект с обязательным полем type
const addCounter = {
...
создаем store
у store есть несколько методов getState (получить состояние), dispatch (изменить состояние) и subscribe (подписаться на изменения состояния)
...
//создал редьюсер в отдельном файле. там изначальный стэйт и функция, которая принимает стэйт, экшн и возвращает стэйт
...
Затем результат крийэтора передается в функцию dispatch():
...
в файле actionTypes мы просто определяем константы 
в файле actions мы импортируем эти константы и  описываем action creators
в компоненте мы описываем mapDispatchToProps и записываем в кастомнйю функцию метод dispatch и передаем action creator
в месте где мы раньше описывали метод сетстэйт мы теперь метод из mapDispatchToProps
...
###  104 Страница теста. Часть 2
тут работа над событиями клика по кнопкам выбора ответа.

Если нам внутри функции надо работать со стэйтом, редакс thunk предусмотрел передачей вторым параметром метода getState
export function quizAnswerClick(answerId) {
    return (dispatch, getState) => {
        const state = getState().quiz


!Везде где используется сет стэйт, мы меняем на диспатч с экшном


### 105 Страница создания тестов
на этой странице будем комбинировать редакс и локальный стейт, так как в локальном стейте постоянно идет валидация, и нелогично все переносить в стор Оо
в локальном стейте мы оставить все кроме quiz. это массив с созданными тестами
