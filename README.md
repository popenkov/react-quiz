...
REDUX
Это абстракция для работы с данными 
Это идеалогия формирования данных на фронтенде, никак не связанная с реакт.
npm install redux
...
//Reducer - это жc функция, которая делает преобразования
//state - объект, который описывает состояние всего приложения
const reducer = (state = initialState, action) => {
  ...
  //мы всегда должны возвращать новый стэйт
  return state;
}

//Store - место, где хранятся все данные. Стэйт для всего приложения
const store = redux.createStore(reducer) //внутрь передаем reducer
console.log(store.getState()) // посмотреть состояние стора.

//Actions - для изменения состояния стора. объект с обяхательным полем type
const addCounter = {
  type: 'ADD',//придумываем сами. помогает определить, какое действие мы сейчас совершаем
...
в редюсере мы описали изначальное состояние стэйта.
